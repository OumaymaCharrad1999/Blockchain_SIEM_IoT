...
func (t *SimpleChaincode) CreateAsset(ctx contractapi.TransactionContextInterface, assetID, message string, severity int, owner string, facility int) error {
...
func (t *SimpleChaincode) InitLedger(ctx contractapi.TransactionContextInterface) error {
	assets := []Asset{
		...
		*/
		{DocType: "asset",ID: "asset1", Message: "failed Login", Severity: 5, Owner: "Tomoko", Facility: 10},
		{DocType: "asset",ID: "asset2", Message: "Uncorect Message", Severity: 5, Owner: "Brad", Facility: 15}
	...
	for _, asset := range assets {
		...
		err := t.CreateAsset(ctx, asset.ID, asset.Message, asset.Severity, asset.Owner, asset.Facility)
		if err != nil {
			return err
		}
	}
